// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/api/v1/acoes": {
            "get": {
                "description": "Get All Ação BR",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Get All Acão BR",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new acao",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Create a acao",
                "parameters": [
                    {
                        "description": "AcaoBrRequest info",
                        "name": "acao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.AcaoBrRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/acoes/setor/{setor}": {
            "get": {
                "description": "Get Lista de Ações de um setor",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Get Lista de Ações de um setor",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "AcaoBr Setor",
                        "name": "setor",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/acoes/{codigo}": {
            "get": {
                "description": "Get a Ação BR by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Get a Acão BR",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "AcaoBr Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Update a acao by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Update a acao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "AcaoBr codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "AcaoBrRequest info",
                        "name": "acao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.AcaoBrRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a acao by codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Delete a acao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "AcaoBr codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/bdrs": {
            "get": {
                "description": "Get All BDR",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "bdrs"
                ],
                "summary": "Get All BDR",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.BdrResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new bdr",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "bdrs"
                ],
                "summary": "Create a bdr",
                "parameters": [
                    {
                        "description": "BdrRequest info",
                        "name": "bdr",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.BdrRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.BdrResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/bdrs/{id}": {
            "get": {
                "description": "Get a BDR by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "bdrs"
                ],
                "summary": "Get a BDR",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Bdr ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.BdrResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Update a bdr by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "bdrs"
                ],
                "summary": "Update a bdr",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Bdr ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "BdrRequest info",
                        "name": "bdr",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.BdrRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.BdrResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a bdr by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "bdrs"
                ],
                "summary": "Delete a bdr",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Bdr ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/carteiras": {
            "get": {
                "description": "Get All Carteiras",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Get All Carteiras",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new carteira",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Create a carteira",
                "parameters": [
                    {
                        "description": "CarteiraRequest info",
                        "name": "carteira",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.CarteiraRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/carteiras/{id}": {
            "get": {
                "description": "Get a Carteira by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Get a Carteira",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Carteira ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Update a carteira by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Update a carteira",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Carteira ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "CarteiraRequest info",
                        "name": "carteira",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.CarteiraRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a carteira by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Delete a carteira",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Carteira ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/carteiras/{id}/ativo": {
            "post": {
                "description": "Add ativo na carteira",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Add ativo na carteira",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Carteira ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "CarteiraAtivoRequest info",
                        "name": "carteira",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.CarteiraAtivoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraAtivoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/carteiras/{id}/ativo/{codigo}": {
            "delete": {
                "description": "Delete a carteira by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "carteiras"
                ],
                "summary": "Delete a carteira",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Carteira ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Ativo Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/comparador": {
            "get": {
                "description": "Compare List of Acoes",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "acoes"
                ],
                "summary": "Compare List of Acoes",
                "parameters": [
                    {
                        "type": "array",
                        "items": {
                            "type": "string"
                        },
                        "collectionFormat": "csv",
                        "description": "Lista de Códigos das Ações",
                        "name": "codigos",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrComparadorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/consolidacoes": {
            "get": {
                "description": "Get All consolidacao BR",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "consolidacoes"
                ],
                "summary": "Get All consolidacao BR",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ConsolidacaoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/consolidacoes/calcular": {
            "post": {
                "description": "Calcular a consolidacao",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "consolidacoes"
                ],
                "summary": "Calcular a consolidacao",
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ConsolidacaoResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/consolidacoes/{codigo}": {
            "get": {
                "description": "Get a consolidacao BR by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "consolidacoes"
                ],
                "summary": "Get a consolidacao BR",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Consolidacao Codigo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ConsolidacaoResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/cotacoes": {
            "get": {
                "description": "Get All cotacao",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Get All cotacao",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Criar a cotacao",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Criar a cotacao",
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/cotacoes/brapi": {
            "post": {
                "description": "Criar a cotacao",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Criar a cotacao",
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/cotacoes/carteira/{carteiraId}": {
            "post": {
                "description": "Criar cotações de ativos em uma carteira",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Criar cotações de ativos em uma carteira",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Carteira ID",
                        "name": "carteiraId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/cotacoes/{codigo}": {
            "get": {
                "description": "Get a cotacao by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Get a cotacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Cotacao Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "data",
                        "name": "data",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/cotacoes/{codigo}/last": {
            "get": {
                "description": "Get a cotacao by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Get a cotacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Cotacao Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/cotacoes/{id}": {
            "delete": {
                "description": "Delete a Cotacao by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "cotacoes"
                ],
                "summary": "Delete a Cotacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Cotacao ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/dividendos": {
            "get": {
                "description": "Get All Dividendo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dividendos"
                ],
                "summary": "Get All Dividendo",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.DividendoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new dividendo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dividendos"
                ],
                "summary": "Create a dividendo",
                "parameters": [
                    {
                        "description": "DividendoRequest info",
                        "name": "dividendo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.DividendoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.DividendoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/dividendos/{codigo}": {
            "get": {
                "description": "Get a dividendos by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dividendos"
                ],
                "summary": "Get dividendos pelo codigo",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Dividendo Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/dividendos/{id}": {
            "delete": {
                "description": "Delete a dividendo by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "dividendos"
                ],
                "summary": "Delete a dividendo",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Dividendo ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/fundos-imobiliarios": {
            "get": {
                "description": "Get All Fundos Imobiliarios",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "fundos-imobiliarios"
                ],
                "summary": "Get All Fundos Imobiliarios",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.FundoImobiliarioResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Create a new Fundo Imobiliario",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "fundos-imobiliarios"
                ],
                "summary": "Create a Fundo Imobiliario",
                "parameters": [
                    {
                        "description": "FundoImobiliarioRequest info",
                        "name": "fundoImobiliario",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.FundoImobiliarioRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.FundoImobiliarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/fundos-imobiliarios/{id}": {
            "get": {
                "description": "Get a Fundo Imobiliario by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "fundos-imobiliarios"
                ],
                "summary": "Get a Fundo Imobiliario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "FundoImobiliario ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.FundoImobiliarioResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Update a Fundo Imobiliario by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "fundos-imobiliarios"
                ],
                "summary": "Update a Fundo Imobiliario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "FundoImobiliario ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "FundoImobiliarioRequest info",
                        "name": "fundoImobiliario",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.FundoImobiliarioRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.FundoImobiliarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a Fundo Imobiliario by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "fundos-imobiliarios"
                ],
                "summary": "Delete a Fundo Imobiliario",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "FundoImobiliario ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/graham-formula": {
            "post": {
                "description": "Calculate graham formula",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "graham-formula"
                ],
                "summary": "Calculate graham formula",
                "parameters": [
                    {
                        "description": "GrahamFormulaRequest info",
                        "name": "grahamFormula",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.GrahamFormulaRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.GrahamFormulaResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/graham-formula/{codigo}": {
            "get": {
                "description": "Get All Graham Formula for a code",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "graham-formula"
                ],
                "summary": "Get All Graham Formula for a code",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.GrahamFormulaResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/graham-formula/{codigo}/last": {
            "get": {
                "description": "Get the most Graham Formula for a code",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "graham-formula"
                ],
                "summary": "Get the most recente Graham Formula for a code",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.GrahamFormulaResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/graham-formula/{id}": {
            "delete": {
                "description": "Delete a Graham Formula by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "graham-formula"
                ],
                "summary": "Delete a Graham Formula",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Graham Formula ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/informacoes": {
            "post": {
                "description": "Criar a informacao",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "informacoes"
                ],
                "summary": "Criar a informacao",
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoInformacaoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/informacoes/{codigo}": {
            "get": {
                "description": "Get a informacao by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "informacoes"
                ],
                "summary": "Get a informacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Informacao Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoInformacaoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/informacoes/{codigo}/last": {
            "get": {
                "description": "Get a informacao by Codigo",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "informacoes"
                ],
                "summary": "Get a informacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Informacao Codigo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoInformacaoResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/informacoes/{id}": {
            "delete": {
                "description": "Delete a Informacao by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "informacoes"
                ],
                "summary": "Delete a Informacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Informacao ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/movimentacoes": {
            "get": {
                "description": "Get All movimentacao BR",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "movimentacoes"
                ],
                "summary": "Get All movimentacao BR",
                "parameters": [
                    {
                        "type": "string",
                        "description": "Codigo",
                        "name": "codigo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.MovimentacaoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/movimentacoes/entrada": {
            "post": {
                "description": "Create a new movimentacao de entrada",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "movimentacoes"
                ],
                "summary": "Create a movimentacao de entrada",
                "parameters": [
                    {
                        "description": "MovimentacaoRequest info",
                        "name": "movimentacao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.MovimentacaoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.MovimentacaoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/movimentacoes/saida": {
            "post": {
                "description": "Create a new movimentacao de saida",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "movimentacoes"
                ],
                "summary": "Create a movimentacao de saida",
                "parameters": [
                    {
                        "description": "MovimentacaoRequest info",
                        "name": "movimentacao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.MovimentacaoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.MovimentacaoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/api/v1/movimentacoes/{id}": {
            "get": {
                "description": "Get a movimentacao BR by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "movimentacoes"
                ],
                "summary": "Get a movimentacao BR",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Movimentacao ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.MovimentacaoResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Delete a movimentacao by ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "movimentacoes"
                ],
                "summary": "Delete a movimentacao",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Movimentacao ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/pkg_api_v1.ErrorResponse"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1.ErrorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "type": "string",
                    "example": "Bad Request"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.AcaoBrRequest": {
            "type": "object",
            "properties": {
                "cnpj": {
                    "type": "string"
                },
                "codigo": {
                    "type": "string"
                },
                "nome": {
                    "type": "string"
                },
                "setor": {
                    "type": "string"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.BdrRequest": {
            "type": "object",
            "properties": {
                "cnpj": {
                    "type": "string"
                },
                "codigo": {
                    "type": "string"
                },
                "nome": {
                    "type": "string"
                },
                "setor": {
                    "type": "string"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.CarteiraAtivoRequest": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "movimento": {
                    "type": "string"
                },
                "proporcaoDesejada": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.CarteiraRequest": {
            "type": "object",
            "properties": {
                "descricao": {
                    "type": "string"
                },
                "nome": {
                    "type": "string"
                },
                "proporcaoDesejada": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.DividendoRequest": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCom": {
                    "type": "string"
                },
                "dataPagamento": {
                    "type": "string"
                },
                "tipo": {
                    "type": "string"
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.FundoImobiliarioRequest": {
            "type": "object",
            "properties": {
                "cnpj": {
                    "type": "string"
                },
                "codigo": {
                    "type": "string"
                },
                "nome": {
                    "type": "string"
                },
                "segmento": {
                    "type": "string"
                },
                "tipo": {
                    "type": "string"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.GrahamFormulaRequest": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "data": {
                    "type": "string"
                },
                "lpa": {
                    "type": "number"
                },
                "plEsperado": {
                    "type": "number"
                },
                "precoAtual": {
                    "type": "number"
                },
                "pvpEsperado": {
                    "type": "number"
                },
                "vpa": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_request.MovimentacaoRequest": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "data": {
                    "type": "string"
                },
                "quantidade": {
                    "type": "number"
                },
                "tipoAtivo": {
                    "type": "string"
                },
                "valorUnitario": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrComparadorResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "dividendo": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorDividendoResponse"
                },
                "eficiencia": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorEficienciaResponse"
                },
                "endividamento": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorEndividamentoResponse"
                },
                "informacao": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorInformacaoResponse"
                },
                "rentabilidade": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorRentabilidadeResponse"
                },
                "valuation": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorValuationResponse"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AcaoBrResponse": {
            "type": "object",
            "properties": {
                "cnpj": {
                    "type": "string"
                },
                "codigo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "nome": {
                    "type": "string"
                },
                "setor": {
                    "type": "string"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoDividendoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCalculo": {
                    "type": "string"
                },
                "dividendos": {
                    "type": "number"
                },
                "dy": {
                    "type": "number"
                },
                "id": {
                    "type": "integer"
                },
                "yoc": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoEficienciaResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCalculo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "margemBruta": {
                    "type": "number"
                },
                "margemEbit": {
                    "type": "number"
                },
                "margemEbitda": {
                    "type": "number"
                },
                "margemLiquida": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoEndividamentoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCalculo": {
                    "type": "string"
                },
                "dividaEbit": {
                    "type": "number"
                },
                "dividaEbitda": {
                    "type": "number"
                },
                "dividaPatrimonioLiquido": {
                    "type": "number"
                },
                "id": {
                    "type": "integer"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoInformacaoResponse": {
            "type": "object",
            "properties": {
                "ativoTotal": {
                    "type": "number"
                },
                "codigo": {
                    "type": "string"
                },
                "dataInformacao": {
                    "type": "string"
                },
                "dividaLiquida": {
                    "type": "number"
                },
                "dividendo": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoDividendoResponse"
                },
                "ebit": {
                    "type": "number"
                },
                "ebitda": {
                    "type": "number"
                },
                "eficiencia": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoEficienciaResponse"
                },
                "endividamento": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoEndividamentoResponse"
                },
                "id": {
                    "type": "integer"
                },
                "lucroBruto": {
                    "type": "number"
                },
                "lucroLiquido": {
                    "type": "number"
                },
                "numeroAcoes": {
                    "type": "integer"
                },
                "patrimonioLiquido": {
                    "type": "number"
                },
                "receitaLiquida": {
                    "type": "number"
                },
                "rentabilidade": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoRentabilidadeResponse"
                },
                "valorFirma": {
                    "type": "number"
                },
                "valuation": {
                    "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoValuationResponse"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoRentabilidadeResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCalculo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "roa": {
                    "type": "number"
                },
                "roe": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.AtivoValuationResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCalculo": {
                    "type": "string"
                },
                "evEbit": {
                    "type": "number"
                },
                "evEbitda": {
                    "type": "number"
                },
                "id": {
                    "type": "integer"
                },
                "lpa": {
                    "type": "number"
                },
                "pebit": {
                    "type": "number"
                },
                "pebitda": {
                    "type": "number"
                },
                "pl": {
                    "type": "number"
                },
                "pvp": {
                    "type": "number"
                },
                "vpa": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.BdrResponse": {
            "type": "object",
            "properties": {
                "cnpj": {
                    "type": "string"
                },
                "codigo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "nome": {
                    "type": "string"
                },
                "setor": {
                    "type": "string"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraAtivoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "movimento": {
                    "type": "string"
                },
                "precoAtual": {
                    "type": "number"
                },
                "precoCompra": {
                    "type": "number"
                },
                "proporcaoDesejada": {
                    "type": "number"
                },
                "quantidade": {
                    "type": "number"
                },
                "saldo": {
                    "type": "number"
                },
                "totalAtual": {
                    "type": "number"
                },
                "totalCompra": {
                    "type": "number"
                },
                "variacao": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraResponse": {
            "type": "object",
            "properties": {
                "ativos": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CarteiraAtivoResponse"
                    }
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "nome": {
                    "type": "string"
                },
                "proporcaoDesejada": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorDividendoResponse": {
            "type": "object",
            "properties": {
                "dividendos": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "dy": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "yoc": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorEficienciaResponse": {
            "type": "object",
            "properties": {
                "margemBruta": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "margemEbit": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "margemEbitda": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "margemLiquida": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorEndividamentoResponse": {
            "type": "object",
            "properties": {
                "dividaEbit": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "dividaEbitda": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "dividaPatrimonioLiquido": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorInformacaoResponse": {
            "type": "object",
            "properties": {
                "ativoTotal": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "dividaLiquida": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "ebit": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "ebitda": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "lucroBruto": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "lucroLiquido": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "patrimonioLiquido": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "receitaLiquida": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "valorFirma": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorRentabilidadeResponse": {
            "type": "object",
            "properties": {
                "roa": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "roe": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ComparadorValuationResponse": {
            "type": "object",
            "properties": {
                "evEbit": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "evEbitda": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "lpa": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "pebit": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "pebitda": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "pl": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "pvp": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                },
                "vpa": {
                    "type": "array",
                    "items": {
                        "type": "object",
                        "additionalProperties": {
                            "type": "number"
                        }
                    }
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.ConsolidacaoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lucroMedio": {
                    "type": "number"
                },
                "lucroProporcao": {
                    "type": "number"
                },
                "quantidadeEntrada": {
                    "type": "number"
                },
                "quantidadeSaida": {
                    "type": "number"
                },
                "tipoAtivo": {
                    "type": "string"
                },
                "valorMedioEntrada": {
                    "type": "number"
                },
                "valorMedioSaida": {
                    "type": "number"
                },
                "valorTotalEntrada": {
                    "type": "number"
                },
                "valorTotalSaida": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.CotacaoHistoricoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "data": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.DividendoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "dataCom": {
                    "type": "string"
                },
                "dataPagamento": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "tipo": {
                    "type": "string"
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.FundoImobiliarioResponse": {
            "type": "object",
            "properties": {
                "cnpj": {
                    "type": "string"
                },
                "codigo": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "nome": {
                    "type": "string"
                },
                "segmento": {
                    "type": "string"
                },
                "tipo": {
                    "type": "string"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.GrahamFormulaResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "data": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "lpa": {
                    "type": "number"
                },
                "margemSeguranca": {
                    "type": "number"
                },
                "plEsperado": {
                    "type": "number"
                },
                "precoAtual": {
                    "type": "number"
                },
                "precoJusto": {
                    "type": "number"
                },
                "pvpEsperado": {
                    "type": "number"
                },
                "vpa": {
                    "type": "number"
                }
            }
        },
        "github_com_MarcoADP_Investment-Manager_pkg_api_v1_response.MovimentacaoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "string"
                },
                "data": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "operacao": {
                    "type": "string"
                },
                "quantidade": {
                    "type": "number"
                },
                "tipoAtivo": {
                    "type": "string"
                },
                "valorTotal": {
                    "type": "number"
                },
                "valorUnitario": {
                    "type": "number"
                }
            }
        },
        "pkg_api_v1.ErrorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "type": "string",
                    "example": "Bad Request"
                }
            }
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "",
	Host:             "",
	BasePath:         "",
	Schemes:          []string{},
	Title:            "",
	Description:      "",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
